...
import { addOne, deleteOne, findOne, updateOne } from "../handlers/handlers.js";
...
const privateTasks = catchError(async (req, res, next) => {
  try {
    const tasks = taskModel
      .find({ $and: [{ visible: "private" }, { createdBy: req.user._id }] })
      .populate("category")
      .populate("category", "-_id name")
      .populate("listTask", "-_id name");
...
const OnePrivateTask = catchError(async (req, res, next) => {
  try {
    const tasks = taskModel
      .findOne({
        _id: req.params.id,
        $and: [{ visible: "private" }, { createdBy: req.user._id }],
      })
      .populate("category")
      .populate("category", "-_id name")
      .populate("listTask", "-_id name");
